...
export default function DashboardNavigation({ routes }: { routes: Route[] }) {
  ...
  // Initialize with all routes that have sub-routes
  const [openCollapsibles, setOpenCollapsibles] = useState<Set<string>>(
    () =>
      new Set(
        routes.filter((route) => route.subs?.length).map((route) => route.id)
      )
  );

  const toggleCollapsible = (routeId: string) => {
        ...
        return (
          <SidebarMenuItem key={route.id}>
            {hasSubRoutes ? (
              <Collapsible
                open={isOpen}
                onOpenChange={() => toggleCollapsible(route.id)}
                className="w-full"
              >
